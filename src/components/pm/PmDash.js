...
const PmDash = () => {
  ...
  useEffect(() => {
    ...
    const fetchAssigned = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/projects/assigned/${userId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const data = await res.json();
  ...
  useEffect(() => {
    const fetchManpower = async () => {
      ...
      try {
        const res = await fetch('http://localhost:5000/api/manpower-requests/mine', {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (!res.ok) throw new Error('Failed to fetch manpower requests');
        const data = await res.json();
                ...
                manpowerRequests
                  ...
                  .map(request => (
                    ...
                    >
                      ...
                      <div className="request-details">
                        <h4>
                          {request.manpower?.map(m => `${m.type} (${m.count})`).join(', ') || 'No Manpower'}
                        </h4>
                        <div className="request-meta">
                          <span>{request.createdBy?.name || 'Unknown'} · {request.project?.projectName || ''}</span>
                          <span>{new Date(request.createdAt).toLocaleDateString()}</span>
