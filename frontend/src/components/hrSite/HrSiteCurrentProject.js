...
/* ---------- Build absolute signing endpoint from axios baseURL ---------- */
const API_BASE = (api?.defaults?.baseURL || '').replace(/\/+$/, '');
const SIGN_ENDPOINT = `${API_BASE}/photo-signed-url`;

async function openSignedPath(path) {
  try {
    const res = await fetch(`${SIGN_ENDPOINT}?path=${encodeURIComponent(path)}`, {
      credentials: 'include',
    });
...
/* ---------- Mention helpers (row-level highlight like Teams) ---------- */
...
/* ---------- document meta readers (tolerant to API field names) ---------- */
function readUploadedBy(doc) {
  ...
  const nestedCandidates = doc.uploadedBy || doc.uploader || doc.addedBy || doc.owner || doc.createdBy || doc.user;
  if (nestedCandidates && typeof nestedCandidates === 'object') {
    const n = nestedCandidates.name || nestedCandidates.fullName || nestedCandidates.username || nestedCandidates.email;
  ...
  for (const c of stringCandidates) {
    if (typeof c === 'string' && c.trim() && !looksLikeObjectId(c)) {
      return c.trim();
    }
  }
...
/* ===========================================================
   HR - Site Current Project (Teams-like + Upload + Delete)
   =========================================================== */
...
const userId = user?._id || user?.id || null; // tolerant
  ...
  const canUploadOrDelete = useMemo(() => {
    ...
    const allowed = new Set([
      'Person in Charge', 'Area Manager', 'Project Manager', 'Staff', 'HR - Site',
    ]);
    const roleName = (
      user.role || user.userType || user.position || user.designation || ''
    ).toString().trim();
  ...
  // join room when Discussions active
  useEffect(() => {
  ...
  // Files: signed URLs (works for string path OR object with {path})
  useEffect(() => {
    let intervalId;
  ...
  const handleTextareaInput = (e) => {
    ...
    if (match) {
      ...
      setMentionDropdown({
        ...
        options,
        query,
        position: { top: rect.top - 150, left: rect.left + 10 }
      });
    } else {
  ...
  const handlePrepareUpload = (files) => {
    ...
    const dups = [];
    files.forEach(f => {
      if (existing.has((f.name || '').toLowerCase())) dups.push(f.name);
    });
  ...
  /* ---------- Drag & Drop helpers (composer + replies) ---------- */
  ...
  const onDragOverComposer = (e) => {
    e.preventDefault();
    setIsDragOver(true);
  };
  const onDragLeaveComposer = (e) => {
    e.preventDefault();
    setIsDragOver(false);
  };
                    ...
                    <table className="files-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                      ...
                      <tbody>
                        {project.documents.map((docItem, idx) => {
                          ...
                          const uploadedBy = readUploadedBy(
                            typeof docItem === 'object' ? docItem : null,
                            path
                          );

                          const uploadedAt = readUploadedAt(
                            typeof docItem === 'object' ? docItem : null,
                            path
                          );
