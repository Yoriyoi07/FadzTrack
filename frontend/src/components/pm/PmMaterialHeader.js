...
import { FaTachometerAlt, FaComments, FaBoxes, FaUsers, FaProjectDiagram } from 'react-icons/fa';
import '../style/pm_style/Pm_Dash.css';

const navItems = [
  { to: '/pm', icon: <FaTachometerAlt />, label: 'Dashboard' },
  { to: '/pm/chat', icon: <FaComments />, label: 'Chat' },
  { to: '/pm/request/:id', icon: <FaBoxes />, label: 'Material' },
  { to: '/pm/manpower-list', icon: <FaUsers />, label: 'Manpower' },
  { to: '/pm/viewprojects/:id', icon: <FaProjectDiagram />, label: 'Projects' }
];

const PmMaterialHeader = ({ title, subtitle }) => {
  const [isHeaderCollapsed, setIsHeaderCollapsed] = useState(false);
  const [profileMenuOpen, setProfileMenuOpen] = useState(false);
  const user = (()=>{ try { return JSON.parse(localStorage.getItem('user'))||null; } catch { return null; } })();
  const userName = user?.name || 'PM';
...
export default PmMaterialHeader;
