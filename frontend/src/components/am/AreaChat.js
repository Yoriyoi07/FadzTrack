...
const AreaChat = () => {
  ...
  useEffect(() => {
    ...
    // if any chat’s lastMessage gets seen, update sidebar
  ...
  // 4) auto-open chat based on URL
  useEffect(() => {
    if (chatId && chats.length) {
      const c = chats.find(ch => ch._id === chatId);
      if (c) {
        setSelectedChat(c);
        setTick(t => t + 1);
      }
  ...
  // 5) join & fetch messages when selectedChat or tick changes
  useEffect(() => {
  ...
  const sendMessage = async () => {
    ...
    await api.post(
      '/messages',
      { conversationId: selectedChat._id, sender: userId, content, type: 'text' },
      { headers }
    );
  ...
  const list = searchQuery.trim() ? searchResults : chats;
      ...
      {showGroupModal && (
        <div className="modal-overlay">
          <div className="modal">
            ...
            <label htmlFor="group-name" className="group-name-label">
              Group Name
            </label>
            <input
              id="group-name"
            ...
            <label htmlFor="group-user-search" className="search-label">
              Search users
            </label>
            <input
            ...
            {list.length === 0 ? (
              ...
              list.map(item => {
                ...
                const preview = conv
                  ...
                  : other.email;
                const timeStr =
                  conv && item.lastMessage?.timestamp
                    ? formatDateTime(item.lastMessage.timestamp)
                    : '';

                return (
                  <div
                    key={conv ? item._id : other._id}
                    className={`modern-chat-item ${
                      conv && selectedChat?._id === item._id ? 'active' : ''
                    }`}
          ...
          <div className="modern-chat-wrapper">
                    ...
                    <div className="modern-chat-header-left">
                      <div className="modern-chat-header-avatar">
                        {selectedChat.isGroup
                          ? <FaUsers />
                          : selectedChat.users.find(u => u._id !== userId).name[0].toUpperCase()}
                      </div>
                          ...
                          >
                            <div className={`modern-message ${msg.isOwn ? 'own' : 'other'}`}>
                              ...
                              {isLastOwn && seenByRecipient && (
                                <div className="seen-indicator"><FaCheck /> Seen</div>
                              )}
            ...
            {selectedChat && showInfoSidebar && (
              <div className="modern-contact-info">
                ...
                {selectedChat.isGroup ? (
                  <p>
                    <strong>Members:</strong> {selectedChat.users.map(u => u.name).join(', ')}
                  </p>
                ) : (
                  <>
                    <div>
