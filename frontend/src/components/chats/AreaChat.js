...
import { FaUsers, FaPaperPlane, FaCheck, FaTachometerAlt, FaComments, FaBoxes, FaEye, FaClipboardList, FaChartBar, FaCalendarAlt, FaProjectDiagram } from 'react-icons/fa';
...
const AreaChat = () => {
  ...
  // Project (adapted for Area Manager)
  ...
  // ── Project for Area Manager (adapted)
  useEffect(() => {
    if (!token || !userId) return;
    (async () => {
      try {
        const { data } = await api.get(`/projects/assigned/allroles/${userId}`, { headers });
        // For area managers, we get an array of projects, so we'll use the first one
  ...
  return (
    <div className="area-chat-wrapper">
      ...
      {/* Modern Header - Adapted for Area Manager */}
      <header className={`dashboard-header ${isHeaderCollapsed ? 'collapsed' : ''}`}>
        ...
        <div className="header-bottom">
          <nav className="header-nav">
            ...
            <Link to="/am/chat" className="nav-item active">
              <FaComments />
